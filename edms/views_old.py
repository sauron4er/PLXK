            ...
            if phase_info[0]['is_approve_chained']:
                ...
                if doc_recipient_emp_seat_id:
                    ...
                    # Якщо ні, фазу не змінюємо, відправляємо керівнику, замінюємо ід отримувача листа
                        ...
                        present_phase_id = Doc_Type_Phase.objects.values_list('id', flat=True)\
                            .filter(document_type_id=doc_type).filter(phase=phase_number-1)[0]
                        post_mark_demand(doc_request, chief['emp_seat_id'], present_phase_id, phase_info[0]['mark_id'])
...
def post_modules(doc_request, doc_files, new_path):
    try:
        ...
        if 'recipient_chief' in doc_modules:
            ...
            recipients.append({'id': doc_modules['recipient_chief']})
        ...
        if 'acquaint_list' in doc_modules:
            ...
            for acquaint in doc_modules['acquaint_list']:
                recipients.append({'id': acquaint['id']})
...
def edms_get_doc(request, pk):
    ...
    if request.method == 'GET':
        ...
        for module in type_modules:
            ...
            elif module['module'] == 'recipient_list':
                test = 'test'
