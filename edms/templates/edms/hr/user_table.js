...
class UserTable extends React.Component {
  ...
  state = {
    ...
    today:
      ...
      (new Date().getMonth() + 1) +
      "-" +
      new Date().getDate(), // сьогоднішня дата
  ...
  onChange(event) {
    if (event.target.name === "acting") {
      ...
      const selectedIndex = event.target.options.selectedIndex;
      this.state.new_emp_id = event.target.options[selectedIndex].getAttribute(
        "data-key"
      );
      this.state.new_emp = event.target.options[selectedIndex].getAttribute(
        "value"
      );
      ...
      axios({
        ...
        .then(response => {
          this.props.changeLists(
            "emps",
            this.props.emps.filter(emp => emp.id !== id)
          );
        })
        .catch(error => {
          console.log("errorpost: " + error);
  ...
  render() {
    ...
    const users_columns = [{ name: "emp", title: "Співробітники" }];
    ...
    return (
      <Fragment>
        <button
          type="button"
          className="btn btn-outline-secondary mb-1 invisible"
        >
          Співробітники:
        </button>
        <DxTable
