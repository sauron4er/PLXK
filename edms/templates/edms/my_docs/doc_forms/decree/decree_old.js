...
class Decree extends React.Component {
  ...
  componentDidMount() {
    if (this.state.seat_list.length === 0) {
      axios({
        method: 'get',
        url: 'get_seats/',
        headers: {
  ...
  // первіряє, чи всі дані внесено і запускає postDecree
  addDecree = (e, type) => {
    e.preventDefault();
  ...
  postDecree = (e, type) => {
    e.preventDefault();
    // якщо користувач обрав посаду для погодження але не нажав "+", робимо це за нього:
    if (this.state.select_approval_seat !== '') {
      // TODO Цей код спрацьовує пізно, вже після того, як форма відправляється у бд.
      this.addNewApprovalSeat(e);
    }
    ...
    if (articles.state.articles.length === 0) {
      ...
      // інфа для форм нових відповідальних відділів та погоджуючих
      formData.append('articles', articles.state.articles);
      formData.append('approval_seats', this.state.approval_seats);
      ...
      console.log(this.state.approval_seats);
      this.onCloseModal(e);
    }
  };
      ...
      return <Modal visible={this.state.open} width='45%' effect="fadeInUp" >
          <div className='css_modal_scroll'>
              ...
              <div className="modal-footer">
                <button className="float-sm-left btn btn-sm btn-outline-success mb-1" id='draft' onClick={this.addDecree}>
