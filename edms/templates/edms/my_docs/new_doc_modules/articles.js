...
class Articles extends React.Component {
  ...
  onChange = event => {
    if (event.target.name === "select_article_dep") {
      ...
      this.setState({
        select_article_dep_id: event.target.options[selectedIndex].getAttribute(
          "data-key"
        ),
        select_article_dep: event.target.options[selectedIndex].getAttribute(
          "value"
        )
      });
    }
    if (event.target.name === "select_edit_article_dep") {
      ...
      const selectedIndex = event.target.options.selectedIndex;
      this.setState({
        select_edit_article_dep_id: event.target.options[
          selectedIndex
        ].getAttribute("data-key"),
        select_edit_article_dep: event.target.options[
          selectedIndex
        ].getAttribute("value")
      });
      ...
      const selectedIndex = event.target.options.selectedIndex;
      this.setState({
        edit_article_dep_id: event.target.options[selectedIndex].getAttribute(
          "data-key"
        ),
        edit_article_dep: event.target.options[selectedIndex].getAttribute(
          "value"
        )
      });
    } else {
  ...
  addNewArticleDep = e => {
    ...
    if (this.state.select_article_dep !== "") {
      ...
      article_deps.push({
        id: this.state.select_article_dep_id,
        dep: this.state.select_article_dep
      });
  ...
  delNewDep = (e, dep_id) => {
    ...
    this.setState(prevState => ({
      new_article_deps: prevState.new_article_deps.filter(
        dep => dep.id !== dep_id
      )
    }));
  };
  ...
  addArticle = e => {
    ...
    if (this.state.new_article_text !== "") {
      ...
      const new_article = {
        ...
        deps: article_deps,
        deadline:
          this.state.new_article_deadline === ""
            ? null
            : this.state.new_article_deadline
      };
      this.setState(prevState => ({
  ...
  addEditArticleDep = e => {
    ...
    if (this.state.select_edit_article_dep !== "") {
      ...
      article_deps.push({
        id: this.state.select_edit_article_dep_id,
        dep: this.state.select_edit_article_dep
      });
      const unique_deps = uniqueArray(article_deps);
      this.setState({
  ...
  delEditDep = (e, dep_id) => {
    e.preventDefault();
    this.setState(prevState => ({
      edit_article_deps: prevState.edit_article_deps.filter(
        dep => dep.id !== dep_id
      )
    }));
  };
  ...
  saveArticle = e => {
    ...
    new_articles_list[
      this.state.edit_article_index
    ].text = this.state.edit_article_text;
    new_articles_list[this.state.edit_article_index].deps = article_deps;
    new_articles_list[
      this.state.edit_article_index
    ].deadline = this.state.edit_article_deadline;
          ...
          <div className="border border-info rounded p-1 mb-2">
            ...
            <div className="d-flex">
              <textarea
                ...
                value={this.state.new_article_text}
                name="new_article_text"
            ...
            <If condition={!this.state.new_article_deadline_area}>
              <button
                className="btn btn-sm btn-outline-secondary mt-1 ml-1"
                onClick={this.showDeadlineArea}
              >
            ...
            <If
              condition={
                this.state.new_article_dep_area &&
                this.state.dep_list.length > 0
              }
            >
              <div className="d-flex align-items-start mt-1">
                <label
                  className="flex-grow-1 text-nowrap mr-1"
                  htmlFor="select_article_dep"
                >
          ...
          <div className="border border-info rounded p-1">
            ...
            <div className="d-flex">
              <div className="mr-1 font-weight-bold">
                {this.state.edit_article_index + 1}.
              </div>
              <textarea
                className="flex-grow-1 form-control"
            ...
            <div className="d-flex align-items-start mt-1">
              ...
              <button
                className=" btn btn-sm btn-outline-secondary font-weight-bold ml-1"
                onClick={this.addEditArticleDep}
              >
                <FontAwesomeIcon icon={faPlus} />
              </button>
            </div>
            ...
            <label className="d-flex">
              ...
              <button
                className="btn btn-sm btn-outline-secondary font-weight-bold align-self-start ml-1"
                onClick={this.clearEditDeadline}
              >
                <FontAwesomeIcon icon={faTimes} />
              </button>
            </label>
